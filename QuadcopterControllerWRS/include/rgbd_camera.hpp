...
public:
  ...
  void publishCloud(cnoid::BodyPtr io_body,cnoid::Link* device_link,imu::imu_manager &imu_manager,double time_step){
    static int count = 0;
...
//        ioBody->findDevice<Camera>("Camera")->link()->position().translation();
    ...
    for (const auto& p : cnoid_points) {
      ...
      if (!isinf(norm) && !isnan(norm)) {
        ...
        const Eigen::Vector3d point = rot * mirror_xy * axis_angle2 * relative + xyz;
        cloud_msg_->points.push_back(pcl::PointXYZ(point[0], point[1], point[2]));
      }
    }
